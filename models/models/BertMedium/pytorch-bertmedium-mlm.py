...
# ===---------------------------------------------------------------------------
#
# Canonical (official-implementation) reference for the BertMedium e2e workload.
#
# Runs `prajjwal1/bert-medium` (8 layers, hidden 512, intermediate 2048, 8 heads,
# hidden_act=gelu erf form) with the upstream HuggingFace transformers package
# on the fixed sentence below and writes the *complete* masked-LM logits tensor,
# not just an argmax:
#
#   reference/bertmedium_mlm_logits_f32.bin  raw little-endian float32, C order
...
#
#   python3 pytorch-bertmedium-mlm.py --write-reference       # generate
#   python3 pytorch-bertmedium-mlm.py --check <candidate.bin> # verify
#
# ===---------------------------------------------------------------------------
...
# the sentence is exactly SEQ_LEN tokens once [CLS]/[SEP] are added. Same
# sentence / SEQ_LEN as the Bert-family MLM precedents (shared 30522 vocab).
...
def fixed_inputs(tokenizer) -> dict[str, torch.Tensor]:
    """Tokenize the fixed sentence into the model's three int64 inputs.

    Insertion order is the runtime-input order of the traced graph:
    `import-bertmedium.py` expands this dict as kwargs, so BERT-family
    `token_type_ids` sits between the other two, mirroring the `Bert`
    precedent's driver call order.
    """
    encoded = tokenizer(
        SENTENCE,
...
def run_reference(model_id: str) -> tuple[np.ndarray, dict, dict]:
    ...
    model = BertForMaskedLM.from_pretrained(model_id).eval()
    ...
    meta = {
        ...
        "token_type_ids": inputs["token_type_ids"].reshape(-1).tolist(),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="BertMedium masked-LM canonical reference generator / checker"
    )
    ...
    reference_path = args.reference_dir / "bertmedium_mlm_logits_f32.bin"
