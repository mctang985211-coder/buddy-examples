...
# ===---------------------------------------------------------------------------
#
# Ahead-of-time importer for the BertMedium (prajjwal1/bert-medium) e2e
# workload, model side only. Same contract as the Bert / BertSmall precedents:
#
...
#
# BertMedium is the "medium" Bert variant: 8 hidden layers, hidden size 512 /
# intermediate 2048, 8 heads, hidden_act=gelu (the erf-exact form), 30522-wide
# masked-LM logits with tied embedding/decoder weights — the same repo layout
# (config.json + pytorch_model.bin + vocab.txt) as bert-tiny / bert-mini /
# bert-small. Architecturally identical to bert-small except the layer count
# (4 -> 8).
#
...
# the canonical reference written by pytorch-bertmedium-mlm.py.
#
# ===---------------------------------------------------------------------------
...
def load_reference_module():
    ...
    path = HERE / "pytorch-bertmedium-mlm.py"
    spec = importlib.util.spec_from_file_location("bertmedium_reference", path)
...
def jit_check(model, inputs, tokenizer, dump_candidate=None) -> int:
    ...
    reference = load_reference_module()
    reference_path = reference.DEFAULT_REFERENCE_DIR / "bertmedium_mlm_logits_f32.bin"
    if not reference_path.is_file():
        raise SystemExit(
            f"missing canonical reference {reference_path}; run "
            "python3 pytorch-bertmedium-mlm.py --write-reference first"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AOT importer for the BertMedium e2e workload"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "pytorch-bertmedium-mlm.py --check can re-verify it independently",
    )
