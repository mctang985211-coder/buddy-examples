...
//===----------------------------------------------------------------------===//
//
// Model-side driver for the BertMedium (prajjwal1/bert-medium) e2e workload.
//
// The signature below is the one emitted by import-bertmedium.py into
// forward.mlir:
//
...
// pytorch-bertmedium-mlm.py, so the complete logits tensor this driver writes
// to `bertmedium_logits_f32.bin` can be checked with
//
//   python3 pytorch-bertmedium-mlm.py --check <dir>/bertmedium_logits_f32.bin
//
...
// Sizes of the AOT artifacts emitted by import-bertmedium.py.
constexpr int64_t kFloat32Params = 41404730;
...
// points pytorch-bertmedium-mlm.py commits into reference/reference_manifest.json).
// Measured from the official implementation; hardcoded expectation.
const int64_t kExpectedArgmaxIds[kSeqLen] = {
    3899, 1996, 4248, 2829, 4419, 14523, 2058, 1996, 13971, 3899, 1010,
};
...
int main(int argc, char **argv) {
  const std::string title = "BERT-medium Inference Powered by Buddy Compiler";
  std::cout << "\033[33;1m" << title << "\033[0m" << std::endl;

  // Directory holding arg0.data, arg1.data and vocab.txt (import-bertmedium.py
  ...
  // token, matching pytorch-bertmedium-mlm.py's fixed_inputs exactly.
  ...
  const std::string outPath = modelDir + "bertmedium_logits_f32.bin";
  {
