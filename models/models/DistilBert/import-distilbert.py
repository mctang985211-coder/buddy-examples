...
# ===---------------------------------------------------------------------------
#
# Ahead-of-time importer for the DistilBert (distilbert-base-uncased) e2e
# workload, model side only. Same contract as the Bert precedent:
#
...
#
# DistilBERT differences against Bert that matter for the signature:
#   * no `token_type_ids` input (one runtime int64 input less),
#   * 6 hidden layers instead of 12, 768 hidden size unchanged,
#   * masked-LM head (30522-wide logits) instead of a 6-way classifier.
#
...
# the canonical reference written by pytorch-distilbert-mlm.py.
#
# ===---------------------------------------------------------------------------
...
def load_reference_module():
    ...
    path = HERE / "pytorch-distilbert-mlm.py"
    spec = importlib.util.spec_from_file_location("distilbert_reference", path)
...
def jit_check(model, inputs, tokenizer, dump_candidate=None) -> int:
    ...
    reference = load_reference_module()
    reference_path = reference.DEFAULT_REFERENCE_DIR / "distilbert_mlm_logits_f32.bin"
    if not reference_path.is_file():
        raise SystemExit(
            f"missing canonical reference {reference_path}; run "
            "python3 pytorch-distilbert-mlm.py --write-reference first"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AOT importer for the DistilBert e2e workload"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "pytorch-distilbert-mlm.py --check can re-verify it independently",
    )
    ...
    model = DistilBertForMaskedLM.from_pretrained(args.model_id).eval()
