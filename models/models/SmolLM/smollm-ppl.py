...
# ===---------------------------------------------------------------------------
#
# Canonical (official-implementation) reference for the SmolLM e2e workload.
#
# Runs `HuggingFaceTB/SmolLM-135M` (LlamaForCausalLM, model_type=llama: hidden
# 576, 30 layers, 9 query heads, 3 KV heads (GQA), SwiGLU MLP 1536, RMSNorm
# 1e-5, RoPE theta 10000, vocab 49152, tied embeddings, bf16 checkpoint loaded
# float32 here) with the upstream HuggingFace transformers package on the fixed
# sentence below (exactly SEQ_LEN GPT-2 BPE tokens, no padding) and writes the
# *complete* logits tensor, not just an argmax:
#
#   reference/smollm_logits_f32.bin   raw little-endian float32, C order
...
#
#   python3 smollm-ppl.py --write-reference             # generate reference/
#   python3 smollm-ppl.py --check <candidate.bin>       # verify full logits
#   python3 smollm-ppl.py --weights <arg0.data>         # recon-weights ppl
#
# ===---------------------------------------------------------------------------
...
def model_path() -> str:
    """Model id or directory; SMOLLM_135M_MODEL_PATH overrides the hub id."""
...
def recon_ppl_check(weights: Path, expected_ppl: float) -> int:
    ...
    order (import-smollm.py). A wrong packing order would scramble tensors and
    ...
    # Same trace as import-smollm.py: parameter order is the packing order.
...
def main() -> int:
    ...
    reference_path = args.reference_dir / "smollm_logits_f32.bin"
