//===- buddy-minimax-h3-fl2va-main.cpp ------------------------------------===//
//
// MiniMax-H3 FL2VA host runtime: Context-IR API -> Buddy Base 768p ->
// Regenerate-2K API.
//
//===----------------------------------------------------------------------===//
...
void printUsage(const char *argv0) {
  std::cerr
      << "Usage: " << argv0
      << " --mode t2va|fl2va --prompt TEXT [--first IMG] [--last IMG]\n";
}
...
int main(int argc, char **argv) {
  ...
  if (mode != "t2va" && mode != "fl2va")
    throw std::runtime_error("--mode must be t2va or fl2va");
  if (prompt.empty())
    throw std::runtime_error("--prompt is required");
  if (mode == "fl2va" && firstPath.empty() && lastPath.empty())
    throw std::runtime_error("fl2va requires --first and/or --last");

  std::string exampleDir = "./";
#ifdef MINIMAX_H3_FL2VA_EXAMPLE_PATH
  exampleDir = MINIMAX_H3_FL2VA_EXAMPLE_PATH;
#endif
  ...
  std::string contextPath = "h3_context_prompt.txt";
  {
    ...
    if (!firstPath.empty())
      cmd << " --image " << std::quoted(firstPath)
          << " --image-role first_frame";
    if (!lastPath.empty())
      cmd << " --image " << std::quoted(lastPath)
  ...
  {
    const char *modelEnv = std::getenv("MINIMAX_H3_FL2VA_MODEL_PATH");
    std::string model =
        (modelEnv && *modelEnv) ? modelEnv : "MiniMaxAI/MiniMax-H3";
    std::ostringstream cmd;
    cmd << "python3 " << apiPy << " tokenize --model " << std::quoted(model)
        << " --subfolder FL2VA --prompt-file " << contextPath
  ...
  if (mode == "fl2va") {
    // Encode conditioning frames into the latent (full clip tensor with
    // first/last frames set; middle zeros). Caller must provide RGB float bin.
    if (!firstPath.empty() || !lastPath.empty()) {
      std::string frameBin = exampleDir + "/cond_frames.bin";
      if (!std::filesystem::exists(frameBin))
        throw std::runtime_error(
            "fl2va requires cond_frames.bin (1x3xTxHxW float32)");
  ...
  const std::string mp4_768 = "h3_fl2va_768p.mp4";
  writeRawAndMux(framesOut, wavOut, mp4_768);

  // 3) Regenerate-2K
  const std::string mp4_2k = "h3_fl2va_2k.mp4";
  {
    std::ostringstream cmd;
