...
# ===---------------------------------------------------------------------------
#
# Canonical (official-implementation) reference for the BertMini e2e workload.
#
# Runs `prajjwal1/bert-mini` (4 layers, hidden 256, intermediate 1024, 4 heads,
...
# not just an argmax:
#
#   reference/bertmini_mlm_logits_f32.bin  raw little-endian float32, C order
...
#
#   python3 pytorch-bertmini-mlm.py --write-reference       # generate
#   python3 pytorch-bertmini-mlm.py --check <candidate.bin> # verify
#
# ===---------------------------------------------------------------------------
...
# sentence / SEQ_LEN as the DistilBert precedent.
...
def fixed_inputs(tokenizer) -> dict[str, torch.Tensor]:
    ...
    `import-bertmini.py` expands this dict as kwargs, so BERT-family
    `token_type_ids` (absent in DistilBERT) sits between the other two,
    mirroring the `Bert` precedent's driver call order.
    """
    encoded = tokenizer(
        SENTENCE,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="BertMini masked-LM canonical reference generator / checker"
    )
    ...
    reference_path = args.reference_dir / "bertmini_mlm_logits_f32.bin"
