...
# ===---------------------------------------------------------------------------
#
# Ahead-of-time importer for the BertMini (prajjwal1/bert-mini) e2e workload,
...
#   * 4 hidden layers, hidden size 256 / intermediate 1024 instead of 768 / 6
#     layers (the Bert-family "mini" variant of the BertTiny 2x128 layout),
...
# the canonical reference written by pytorch-bertmini-mlm.py.
#
# ===---------------------------------------------------------------------------
...
def load_reference_module():
    ...
    path = HERE / "pytorch-bertmini-mlm.py"
    spec = importlib.util.spec_from_file_location("bertmini_reference", path)
...
def jit_check(model, inputs, tokenizer, dump_candidate=None) -> int:
    ...
    reference = load_reference_module()
    reference_path = reference.DEFAULT_REFERENCE_DIR / "bertmini_mlm_logits_f32.bin"
    if not reference_path.is_file():
        raise SystemExit(
            f"missing canonical reference {reference_path}; run "
            "python3 pytorch-bertmini-mlm.py --write-reference first"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AOT importer for the BertMini e2e workload"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "pytorch-bertmini-mlm.py --check can re-verify it independently",
    )
