...
//===----------------------------------------------------------------------===//
//
// Model-side driver for the BertMini (prajjwal1/bert-mini) e2e workload.
//
// The signature below is the one emitted by import-bertmini.py into
// forward.mlir:
//
//   func.func @forward(%arg0: memref<11201594xf32>,   // packed float32 params
...
//
// Unlike DistilBERT, BERT takes a `token_type_ids` runtime input (the Bert
// precedent's driver order). The fixed sentence and sequence length match
// pytorch-bertmini-mlm.py, so the complete logits tensor this driver writes
// to `bertmini_logits_f32.bin` can be checked with
//
//   python3 pytorch-bertmini-mlm.py --check <dir>/bertmini_logits_f32.bin
//
//===----------------------------------------------------------------------===//
...
// Sizes of the AOT artifacts emitted by import-bertmini.py.
constexpr int64_t kFloat32Params = 11201594;
...
int main(int argc, char **argv) {
  const std::string title = "BERT-mini Inference Powered by Buddy Compiler";
  std::cout << "\033[33;1m" << title << "\033[0m" << std::endl;

  // Directory holding arg0.data, arg1.data and vocab.txt (import-bertmini.py
  ...
  // token, matching pytorch-bertmini-mlm.py's fixed_inputs exactly.
  ...
  const std::string outPath = modelDir + "bertmini_logits_f32.bin";
  {
