...
//===----------------------------------------------------------------------===//
//
// Model-side driver for the SmolLM2 (HuggingFaceTB/SmolLM2-135M) e2e
// workload.
//
// The signature below is the one emitted by import-smollm2.py into
// forward.mlir:
//
...
//
// SmolLM2-135M is LlamaForCausalLM (model_type=llama) with a GPT-2-style
...
// to `smollm2_driver_logits_f32.bin` (distinct from the canonical
// reference/smollm2_logits_f32.bin) can be checked with
//
//   python3 smollm2-ppl.py --check <dir>/smollm2_driver_logits_f32.bin
//
...
// Sizes of the AOT artifacts emitted by import-smollm2.py: 134,515,008
...
// The workload's fixed case; must stay in sync with smollm2-ppl.py / the
// reference manifest: "Once upon a time there was a little girl who lived in
// a small village." tokenized by the official SmolLM2 GPT-2 BPE tokenizer
// (16 ids).
...
const int64_t kExpectedArgmaxIds[kSeqLen] = {
    346, 253, 655, 28, 436, 253, 1838, 8180,
    3365, 4161, 281, 253, 1165, 6560, 281, 2306,
};
...
// Declare the SmolLM2 forward function generated by the buddy frontend.
extern "C" void _mlir_ciface_forward(MemRef<float, 3> *result,
                                     MemRef<float, 1> *arg0,
                                     MemRef<long long, 2> *arg1);

int main(int argc, char **argv) {
  const std::string title = "SmolLM2-135M Inference Powered by Buddy Compiler";
  std::cout << "\033[33;1m" << title << "\033[0m" << std::endl;

  // Directory holding arg0.data and vocab.txt (import-smollm2.py output).
  ...
  /// no C++ byte-level BPE tokenizer for SmolLM2 in buddy's TextContainer).
  ...
  const std::string outPath = modelDir + "smollm2_driver_logits_f32.bin";
  {
