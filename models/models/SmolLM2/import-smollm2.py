...
# ===---------------------------------------------------------------------------
#
# Ahead-of-time importer for the SmolLM2 (HuggingFaceTB/SmolLM2-135M) e2e
# workload, model side only. Same AutoModelForCausalLM + DynamoCompiler
# pathway as the SmolLM / Qwen3 / Pythia precedents (LlamaForCausalLM falls
# into the same shape), same artifact contract as the BertMedium precedent:
#
...
#
# SmolLM2-135M is LlamaForCausalLM (model_type=llama): hidden 576, 30 layers,
# 9 query heads / 3 KV heads (GQA), SwiGLU MLP 1536, RMSNorm 1e-5, RoPE
# theta 100000, max positions 8192, vocab 49152, tied embeddings. (Checkpoint
# storage dtype: see the measured note in HANDOFF.md — this snapshot's
# model.safetensors stores bfloat16 tensors, unlike the in-repo SmolLM-135M
# precedent whose hub snapshot stores float32 despite the same bf16 config
# declaration; the fp32 load below converts bf16 -> float32 exactly.)
...
# expected per-position argmax ids come from smollm2-ppl.py / reference/.
#
...
# the canonical reference written by smollm2-ppl.py --write-reference.
#
# ===---------------------------------------------------------------------------
...
def load_reference_module():
    ...
    path = HERE / "smollm2-ppl.py"
    spec = importlib.util.spec_from_file_location("smollm2_reference", path)
...
def build_inputs():
    ...
    The dict insertion order is the traced graph's runtime-input order. SmolLM2
...
def check_param_layout(params) -> None:
    ...
    SmolLM2 packs a single arg0.data blob of float32 parameters (the
    checkpoint is loaded float32 by import-smollm2.py). Any other dtype breaks
...
def write_artifacts(graph, params, output_dir: Path) -> dict:
    ...
    # SmolLM2 convention: every lifted tensor is float32; one concatenated blob.
...
def jit_check(model, input_ids, dump_candidate=None) -> int:
    ...
    reference_path = refmod.DEFAULT_REFERENCE_DIR / "smollm2_logits_f32.bin"
    if not reference_path.is_file():
        raise SystemExit(
            f"missing canonical reference {reference_path}; run "
            "python3 smollm2-ppl.py --write-reference first"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AOT importer for the SmolLM2 e2e workload"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "smollm2-ppl.py --check can re-verify it independently",
    )
