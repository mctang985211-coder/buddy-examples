...
# ===---------------------------------------------------------------------------
#
# Canonical (official-implementation) reference for the Pythia e2e workload.
#
# Runs `EleutherAI/pythia-70m` (GPTNeoXForCausalLM, gpt_neox: hidden 512,
# 6 layers, 8 heads, intermediate 2048, untied embeddings, 50304-wide logits)
# with the upstream HuggingFace transformers package on the fixed sentence
# below (exactly SEQ_LEN BPE tokens, no padding) and writes the *complete*
# logits tensor, not just an argmax:
#
#   reference/pythia_logits_f32.bin   raw little-endian float32, C order
...
#
#   python3 pythia-ppl.py --write-reference             # generate reference/
#   python3 pythia-ppl.py --check <candidate.bin>       # verify full logits
#   python3 pythia-ppl.py --weights <arg0.data>         # recon-weights ppl
#
# ===---------------------------------------------------------------------------
...
# Fixed workload input: tokenizes (GPT-2 byte-level BPE) to exactly SEQ_LEN
# ids without padding — verified by --write-reference, which fails otherwise.
...
def model_path() -> str:
    """Model id or directory; PYTHIA_70M_MODEL_PATH overrides the hub id."""
    import os
...
def recon_ppl_check(weights: Path, expected_ppl: float) -> int:
    ...
    order (import-pythia.py). A wrong packing order would scramble tensors and
    ...
    # Same trace as import-pythia.py: parameter order is the packing order.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Pythia-70M canonical reference generator / checker"
    )
    parser.add_argument(
        "--write-reference",
        action="store_true",
    ...
    reference_path = args.reference_dir / "pythia_logits_f32.bin"
