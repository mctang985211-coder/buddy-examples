...
# ===---------------------------------------------------------------------------
#
# Ahead-of-time importer for the Pythia (EleutherAI/pythia-70m) e2e workload,
# model side only. Same AutoModelForCausalLM + DynamoCompiler pathway as the
# Qwen3 precedent (gpt_neox falls into the same shape), same artifact contract
# as the BertMedium precedent:
#
...
#
# Pythia-70m is GPTNeoXForCausalLM (gpt_neox): hidden 512, 6 layers, 8 heads,
# intermediate 2048, gelu, rotary 25%, untied embeddings, 50304-wide logits.
...
# expected per-position argmax ids come from pythia-ppl.py / reference/.
#
...
# the canonical reference written by pythia-ppl.py --write-reference.
#
# ===---------------------------------------------------------------------------
...
def load_reference_module():
    ...
    path = HERE / "pythia-ppl.py"
    spec = importlib.util.spec_from_file_location("pythia_reference", path)
...
def model_path() -> str:
    return os.environ.get("PYTHIA_70M_MODEL_PATH", MODEL_ID)


def build_inputs():
    """Return (input_ids tensor, input_ids numpy) for the fixed workload text.

    The dict insertion order is the traced graph's runtime-input order. Pythia
...
def check_param_layout(params) -> None:
    ...
    Pythia packs a single arg0.data blob: 70,426,624 trainable float32
    parameters plus the 8-float32 rotary inv_freq buffer (GPT-NeoX computes
    cos/sin from it inside the traced forward). Any other dtype breaks the
    one-blob packing convention and must fail loudly, not be silently packed.
    """
...
def write_artifacts(graph, params, output_dir: Path) -> dict:
    ...
    # Pythia convention: every lifted tensor is float32; one concatenated blob.
...
def jit_check(model, input_ids, dump_candidate=None) -> int:
    ...
    reference_path = refmod.DEFAULT_REFERENCE_DIR / "pythia_logits_f32.bin"
    if not reference_path.is_file():
        raise SystemExit(
            f"missing canonical reference {reference_path}; run "
            "python3 pythia-ppl.py --write-reference first"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AOT importer for the Pythia e2e workload"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "pythia-ppl.py --check can re-verify it independently",
    )
