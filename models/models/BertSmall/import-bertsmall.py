...
# ===---------------------------------------------------------------------------
#
# Ahead-of-time importer for the BertSmall (prajjwal1/bert-small) e2e workload,
# model side only. Same contract as the Bert / BertMini precedents:
#
...
#
# BertSmall is the "small" Bert variant: 4 hidden layers, hidden size 512 /
...
# the canonical reference written by pytorch-bertsmall-mlm.py.
#
# ===---------------------------------------------------------------------------
...
def load_reference_module():
    ...
    path = HERE / "pytorch-bertsmall-mlm.py"
    spec = importlib.util.spec_from_file_location("bertsmall_reference", path)
...
def jit_check(model, inputs, tokenizer, dump_candidate=None) -> int:
    ...
    reference = load_reference_module()
    reference_path = reference.DEFAULT_REFERENCE_DIR / "bertsmall_mlm_logits_f32.bin"
    if not reference_path.is_file():
        raise SystemExit(
            f"missing canonical reference {reference_path}; run "
            "python3 pytorch-bertsmall-mlm.py --write-reference first"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AOT importer for the BertSmall e2e workload"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "pytorch-bertsmall-mlm.py --check can re-verify it independently",
    )
