...
//===----------------------------------------------------------------------===//
//
// Model-side driver for the BertTiny (prajjwal1/bert-tiny) e2e workload.
//
// The signature below is the one emitted by import-berttiny.py into
// forward.mlir:
//
//   func.func @forward(%arg0: memref<4416698xf32>,    // packed float32 params
...
// pytorch-berttiny-mlm.py, so the complete logits tensor this driver writes
// to `berttiny_logits_f32.bin` can be checked with
//
//   python3 pytorch-berttiny-mlm.py --check <dir>/berttiny_logits_f32.bin
//
//===----------------------------------------------------------------------===//
...
// Sizes of the AOT artifacts emitted by import-berttiny.py.
constexpr int64_t kFloat32Params = 4416698;
...
int main(int argc, char **argv) {
  const std::string title = "BERT-tiny Inference Powered by Buddy Compiler";
  std::cout << "\033[33;1m" << title << "\033[0m" << std::endl;

  // Directory holding arg0.data, arg1.data and vocab.txt (import-berttiny.py
  ...
  // token, matching pytorch-berttiny-mlm.py's fixed_inputs exactly.
  ...
  const std::string outPath = modelDir + "berttiny_logits_f32.bin";
  {
