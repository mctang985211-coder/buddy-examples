...
# ===---------------------------------------------------------------------------
#
# Canonical (official-implementation) reference for the BertTiny e2e workload.
#
# Runs `prajjwal1/bert-tiny` (2 layers, hidden 128, hidden_act=gelu erf form)
# with the upstream HuggingFace transformers package on the fixed sentence
# below and writes the *complete* masked-LM logits tensor, not just an argmax:
#
#   reference/berttiny_mlm_logits_f32.bin  raw little-endian float32, C order
...
#
#   python3 pytorch-berttiny-mlm.py --write-reference       # generate
#   python3 pytorch-berttiny-mlm.py --check <candidate.bin> # verify
#
# ===---------------------------------------------------------------------------
...
def fixed_inputs(tokenizer) -> dict[str, torch.Tensor]:
    ...
    `import-berttiny.py` expands this dict as kwargs, so BERT-family
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="BertTiny masked-LM canonical reference generator / checker"
    )
    ...
    reference_path = args.reference_dir / "berttiny_mlm_logits_f32.bin"
